  ...
  return (
    ...
    <div className="bg-black text-white selection:bg-white selection:text-black min-h-screen" style={{ fontFamily: 'Pretendard, -apple-system, sans-serif' }}>
      ...
      <section ref={heroSectionRef} className="relative h-screen h-[100dvh] flex flex-col overflow-hidden">
        ...
        >
          <div className="backdrop-blur-xl bg-black/40 border border-white/10 rounded-[2.5rem] p-8 sm:p-12 md:p-16 shadow-[0_8px_32px_0_rgba(0,0,0,0.6)] flex flex-col items-center max-w-[1040px] w-full transition-transform duration-700 hover:scale-[1.01]">
            {/* 배지 */}
            <motion.div
              className="px-4 py-1.5 rounded-full border border-[#C9A84C]/30 bg-[#C9A84C]/10 backdrop-blur-md mb-6"
              initial={{ opacity: 0, y: 10 }}
              animate={entranceComplete ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            >
              <p className="text-[11px] font-extrabold tracking-[0.28em] uppercase text-[#e5c777]">
                {content.hero.badge}
              </p>
            </motion.div>
            ...
            {/* 메인 헤드라인 */}
            <motion.h1
              className={`apple-white-gradient font-extrabold leading-[1.1] tracking-tight max-w-4xl${lang === 'en' ? ' font-display' : ''}`}
            ...
            >
              ...
              <br />
              <span className="apple-gold-gradient drop-shadow-lg">{content.hero.titleLeft[1]}</span>
            </motion.h1>

            {/* 서브 헤드라인 */}
            <motion.p
              className="mt-6 mb-2 font-bold tracking-tight"
              style={{ fontSize: 'clamp(16px, 2.4vw, 26px)', color: 'rgba(255,255,255,0.7)' }}
              initial={{ opacity: 0, y: 20, filter: 'blur(6px)' }}
              animate={entranceComplete ? { opacity: 1, y: 0, filter: 'blur(0px)' } : {}}
              transition={{ duration: 0.85, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
            >
              {content.hero.titleRight[0]}{' '}
              <span className="text-white drop-shadow-md">{content.hero.titleRight[1]}</span>
            </motion.p>

            {/* 설명 */}
            <motion.p
              className="text-[15px] sm:text-[17px] leading-relaxed max-w-2xl mt-4 font-medium"
